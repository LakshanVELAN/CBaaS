...
def extraction_guide(request):
    ...
    guide = {
        ...
        'extraction_prompt': (
            ...
            '**2. Extract EXACT Route Paths (CRITICAL)**\n'
            '- Every path you extract MUST be a defined route in the app\'s router configuration.\n'
            '- Read App.jsx/tsx and find all `<Route path="...">` or route definition objects.\n'
            '- **Cross-reference every extracted path against the router**: for each sidebar menu item,\n'
            '  page key, accessible_routes entry, and navigates_to value, verify it matches a real route path.\n'
            '- For sidebar menu items: extract the `route` as the EXACT full path from the router definition\n'
            '  (e.g. `/dashboard/resume` not just `resume` or `ResumeManager`).\n'
            '- For `navigates_to` in clickable_actions: must be the exact route path from the router,\n'
            '  not a label or description. If a button navigates but the target route is not in the router,\n'
            '  set `navigates_to` to the actual route path if you can determine it, otherwise omit it.\n'
'- For `accessible_routes`: include the full router path for every route the role can access.\n'
'  E.g. `/students`, `/students/:id`, `/courses`, `/settings/profile`.\n'
'\n'
            '**3. Per Role, extract:**\n'
            '- `label` — human-readable role name\n'
            '- `detection` — the exact JS condition that activates this role\n'
            '- `home_redirect` — the route the user lands on after login (must match a real route path)\n'
            '- `sidebar` — every menu item with label, route (full path), icon, description\n'
            '- `accessible_routes` — array of all route paths this role can visit (must be validated against router)\n'
            '- `pages` — for every route, the key MUST be the exact path as defined in the router.\n'
            '  Extract title, description, visible_content (array of strings), '
            '  and clickable_actions (array of objects with label and navigates_to or action).\n'
'  The navigates_to value MUST be a real route path from the router.\n\n'
            '**4. Also extract:**\n'
...
'- **Every path in the output must be cross-referenced against the router configuration.**\n'
'  If a path does not exist in the router, remove it or flag it with `_comment`.\n\n'
